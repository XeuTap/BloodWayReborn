ranger_stun_arrow = class ({})
LinkLuaModifier("modifier_ranger_stun_arrow", "libraries/abilities/modifier_ranger_stun_arrow", LUA_MODIFIER_MOTION_NONE)


function ranger_stun_arrow:GetCooldown( nLevel )
	return self.BaseClass.GetCooldown( self, nLevel )
end

function ranger_stun_arrow:CastFilterResultTarget( hTarget )
...
function ranger_stun_arrow:GetCustomCastErrorTarget( hTarget )
	if self:GetCaster() == hTarget then
...
function ranger_stun_arrow:OnSpellStart()
...
function ranger_stun_arrow:OnProjectileHit(hTarget, vLocation)
