modifier_huntsman_accurate_shot = class ({})

function modifier_huntsman_accurate_shot:IsHidden()
	return true
end

function modifier_huntsman_accurate_shot:DeclareFunctions()
	local funcs = {
...
function modifier_huntsman_accurate_shot:OnCreated( _kv )
	self:OnRefresh( _kv )
end

function modifier_huntsman_accurate_shot:OnRefresh( _kv )
	local ability = self:GetAbility()
	self.crit_chance = ability:GetSpecialValueFor("crit_chance")
	self.dmg_multiplier = ability:GetSpecialValueFor("crit_damage")
    self.evasion_chance = ability:GetSpecialValueFor("miss_chance")
	self.is_crit = false
end

function modifier_huntsman_accurate_shot:GetModifierEvasion_Constant()
    return self.evasion_chance
end

function modifier_huntsman_accurate_shot:GetModifierPreAttack_CriticalStrike( params )
	if IsServer() then
...
function modifier_huntsman_accurate_shot:OnAttackLanded( params )
	if IsServer() then
