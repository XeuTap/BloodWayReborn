...
function WriteKV(firstLine, t, indent, done)
  ...
  print(string.rep ("\t", indent-1) .. "\"" .. firstLine .. "\"\n")
  print(string.rep ("\t", indent-1) .. "{\n")
  for k,value in pairs(t) do
    if type(value) == "table" and not done[value] then
        done [value] = true
        WriteKV (k, value, indent + 1, done)
      elseif type(value) == "userdata" and not done[value] then
        --skip userdata
      else
        print(string.rep ("\t", indent) .. "\"" .. tostring(k) .. "\"\t\t\"" .. tostring(value) .. "\"\n")
      end
  end
  print(string.rep ("\t", indent-1) .. "}\n")
end
...
function Attachments:start()
  ...
  if IsInToolsMode() then
    ...
    self.doSphere = false
    self.attachDB = LoadKeyValues("scripts/attachments.txt")
    print("Attachment not in tools mode, skipping GUI")
  end
end
...
function Attachments:Attachment_SaveAttach(args)
  ...
  WriteKV("Attachments", Attachments.attachDB)
end
